...
import { ArrowLeft, Users, Receipt, DollarSign, MoreVertical, Edit, Trash2, MessageCircle, LogOut } from "lucide-react"
...
export default function GroupPage() {
  ...
  useEffect(() => {
    ...
    loadGroupData()
  }, [groupId, fetchGroups, fetchParticipants, fetchExpenses, fetchSettlements])
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100">
              ...
              <TabsContent value="balances" className="space-y-6">
                <div className="flex justify-between items-center">
                  ...
                  <div className="space-x-2">
                    <AddParticipantModal groupId={groupId} />
                    <SettleUpModal groupId={groupId} />
                  </div>
                </div>
                  ...
                  {balances.map((balance) => (
                    <Card key={balance.participant_id}>
                      <CardContent className="p-6">
                        <div className="flex items-center justify-between">
                          <div>
                            <h3 className="font-semibold text-lg">{balance.participant_name}</h3>
                  ...
                  {settlements.length === 0 ? (
                    <Card>
                      <CardContent className="flex flex-col items-center justify-center py-12">
                        ...
                        <p className="text-gray-600 text-center mb-4">
                          Settlements will appear here when members pay each other back
                        </p>
                        <SettleUpModal groupId={groupId} />
                      </CardContent>
                    </Card>
                        ...
                        <CardContent className="p-6">
                          <div className="flex items-center justify-between">
                            <div>
                              ...
                              {settlement.note && <p className="text-sm text-gray-600 mt-1">Note: {settlement.note}</p>}
                            </div>
                            <div className="text-2xl font-bold text-green-600">৳{settlement.amount.toFixed(2)}</div>
                          </div>
                        </CardContent>
                      </Card>
                    ))
      ...
      {/* Modals */}
      {editingExpense && (
        <EditExpenseModal expense={editingExpense} groupId={groupId} onClose={() => setEditingExpense(null)} />
      )}

      {deletingExpense && (
        <DeleteExpenseModal expense={deletingExpense} groupId={groupId} onClose={() => setDeletingExpense(null)} />
      )}
    </div>
  )
}
